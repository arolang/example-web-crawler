...
- **The container image base tag is `:latest`.** `ghcr.io/arolang/aro-runtime:latest` is unpinned — a base-image change can shift the runtime under you between builds. <!-- TODO: pin the runtime image to a digest or version once `aro-runtime` publishes one. -->
